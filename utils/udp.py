...
def main():
    ...
    for key, value in maskRCNN.named_parameters():
        if value.requires_grad:
            ...
            if cfg.FAST_RCNN.USE_CASCADE:
                if '_3' in key:
                    stage2_params.append(value)
                    stage2_param_names.append(key)
                elif '_2' in key:
                    stage1_params.append(value)
                    stage1_param_names.append(key)
                elif 'Box_' in key and '_3' not in key and '_2' not in key:
                    stage0_params.append(value)
                    stage0_param_names.append(key)
        else:
    ...
    if cfg.FAST_RCNN.USE_CASCADE:
        params.append({'params': stage0_params, 'lr': 0, 'weight_decay': cfg.SOLVER.WEIGHT_DECAY})
        params.append({'params': stage1_params, 'lr': 0, 'weight_decay': cfg.SOLVER.WEIGHT_DECAY})
        params.append({'params': stage2_params, 'lr': 0, 'weight_decay': cfg.SOLVER.WEIGHT_DECAY})
