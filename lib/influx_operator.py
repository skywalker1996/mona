	...
	def pullData(self, measurement, field, tags=None, time_range='30s'):
		...
		try:
			if(tags != None):
				...
				tables = self.query_api.query('from(bucket:"%s") |> range(start: -%s)'%(self.bucket,time_range)+
										'|> filter(fn: (r) => r._field == "%s")'%(field)+
			...
			for table in tables:
				for record in table.records:
					result = {}
					result['time'] = str(record.values['_time'])
					result[field] = record.values['_value']
...
if __name__ == '__main__':
	...
	results = client.pullData(measurement=measurement, field=field, tags=tags, time_range='5m')
	print(results)
